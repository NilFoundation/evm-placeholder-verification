...
library modular_commitment_scheme_circuit4 {
    ...
    uint256 constant _eta = 850133242367294717247047710337156064369915212762097839080979253061321047676;
    bytes constant point_ids = hex"000100010001000100010001000100010002010002010002010002010002010001000100020100000000020002000000000000000203000203"; // 1 byte -- point id
    bytes constant poly_points_num = hex"001d0010000a0002"; // 2 byte lengths
    bytes constant poly_ids = hex"00000040008000c001000140018001c002000240028002c003000340038003c004000440048004c005000540058005c006000640068006c0070000000040008000c001000140018001c002000240028002c003000340038003c002000240028002c0030003c004c0050006c0070006c00700"; // 2 byte poly_id 2 byte
    ...
    function prepare_eval_points(uint256[unique_points] memory result, uint256 xi, uint256 eta) internal view {
		...
		result[3] = mulmod(xi, field.pow_small(inversed_omega, 3, modulus), modulus);
...
unchecked {
		...
		///* eta points check */
		{
			...
			points[0xe] = basic_marshalling.get_uint256_be(blob,0x448);
			points[0xf] = basic_marshalling.get_uint256_be(blob,0x4a8);
			// Check keccak(points) 
			if ( bytes32(0x4b402a6a2a392ead02fb32e389da6ecd5108c68d8f270f0ad1084b1fc443bb4c) != keccak256(abi.encode(points))) {
				return false;
			}
		}
