...
library modular_commitment_scheme_circuit4 {
    ...
    bytes constant   points_ids = hex"01010101010101010303030303010103000000";
...
unchecked {
    ...
    function prepare_eval_points(uint256[][unique_points] memory result, uint256 xi) internal view {
        uint256 inversed_omega = field.inverse_static(omega, modulus);
		 ...
		 result[4][2] = mulmod(xi, field.pow_small(omega, 5, modulus), modulus);
...
unchecked {
		...
		/* eta points check */
		{
			...
			/* Check keccak(points) */
			if ( bytes32(0x3cceb654ee4eca659357091f56c2baf512833ea985eb092292dc96e177d80c81) != keccak256(abi.encode(points))) {
				return false;
			}
		}
