...
library modular_commitment_scheme_circuit3 {
    ...
    uint256 constant _etha = 18362415859916238774382895683161565527748531679388927232994577019378017026317;
...
unchecked {
    ...
    function prepare_eval_points(uint256[][unique_points] memory result, uint256 xi) internal view {
        uint256 inversed_omega = field.inverse_static(omega, modulus);
		 ...
		 result[4][2] = mulmod(xi, field.pow_small(omega, 4, modulus), modulus);
...
unchecked {
		...
		/* eta points check */
		{
			...
			/* Check keccak(points) */
			if ( bytes32(0x40b6be29932989f5373758bae1cbf33b8fef0cdeaa903eca417f9833fed4c81c) != keccak256(abi.encode(points))) {
				return false;
			}
		}
