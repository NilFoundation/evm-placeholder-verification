...
library basic_marshalling {
    ...
    internal pure returns (uint256 result_offset) {
        //uint256(getBytes32(blob,offset_by))
...
//        require( xi.length == 3 );
    ...
    function verify_proof_be(
        ...
        uint256 offset, 
        uint256[] memory roots,
        uint256[2][2][2] memory evaluation_points,
        types.transcript_data memory tr_state, 
        types.fri_params_type memory fri_params)
    internal view returns (bool result) {
        uint256 ind;
...
interface IGateArgument {
    function evaluate_gates_be(bytes calldata blob,
        ...
        int256[2][2] calldata columns_rotations
    ) external pure returns (uint256 gates_evaluation);
}


// File contracts/zkllvm/arithmetics_output/gate_argument.sol
