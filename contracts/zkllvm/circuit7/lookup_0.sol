...
library lookup_circuit7_0{
    ...
    function evaluate_lookup_0_be(
		...
		selector_value=basic_marshalling.get_uint256_be(blob, 736);
		l = mulmod( 1,selector_value, modulus);
		theta_acc=theta;
		sum = 0;
		prod = basic_marshalling.get_uint256_be(blob, 1152);
		...
		theta_acc = mulmod(theta_acc, theta, modulus);
		sum = 0;
		prod = basic_marshalling.get_uint256_be(blob, 1344);
		sum = addmod(sum, prod, modulus);
