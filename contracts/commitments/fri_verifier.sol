...
library fri_verifier {
    ...
    internal pure returns (uint256 result_offset) {
        ...
        // p
        uint256 value_len;
        assembly {value_len := shr(0xc0,calldataload(add(blob.offset, result_offset)))}
    ...
    internal view returns (uint256 result) {
        {
            uint256 _tmp_result;
            uint256 _tmp_offset = local_vars.round_proof_y_offset;
            uint256 _tmp_i = j;
            assembly {_tmp_result := calldataload(add(blob.offset, add(add(_tmp_offset, 8), mul(_tmp_i, 0x20))))}
            result = _tmp_result;
        }
        if (i == 0) {
    ...
    function parse_verify_round_proof_be(bytes calldata blob, uint256 round_proof_offset, local_vars_type memory local_vars)
    internal pure returns (bool result) {
        require(m == get_round_proof_y_n_be(blob, round_proof_offset), "Round proof y size is not equal to m!");
        ...
        local_vars.round_proof_y_offset = skip_to_round_proof_y_be(blob, round_proof_offset);
        for (uint256 j = 0; j < m;) {
            local_vars.status = merkle_verifier.parse_verify_merkle_proof_be(
                blob,
                local_vars.round_proof_p_offset,
                basic_marshalling.get_i_bytes32_from_vector(blob, local_vars.round_proof_y_offset, j)
            );
    ...
    internal view returns (bool result) {
        ...
        for (uint256 i = 0; i < fri_params.r;) {
            ...
            local_vars.status = parse_verify_round_proof_be(blob, local_vars.round_proof_offset, local_vars);
            if (!local_vars.status) {
                return false;
            }

            // get colinear_value
            local_vars.colinear_value = basic_marshalling.get_uint256_be(blob, local_vars.round_proof_offset);
            if (polynomial.interpolate_evaluate_by_2_points_neg_x(
                    ...
                    local_vars.alpha,
                    fri_params.modulus) != local_vars.colinear_value) {
                return false;
            }
            ...
            if (i < fri_params.r - 1) {
                ...
                transcript.update_transcript_b32_by_offset_calldata(
                    tr_state,
                    blob,
                    skip_to_round_proof_T_root_be(blob, skip_round_proof_be(blob, local_vars.round_proof_offset))
                );
        ...
        if (local_vars.len - 1 > uint256(2)**(field.log2(fri_params.max_degree + 1) - fri_params.r) - 1) {
            return false;
        }

        if (
            polynomial.evaluate_by_ptr(blob, offset + basic_marshalling.LENGTH_OCTETS, local_vars.len,
                                       local_vars.x, fri_params.modulus) != local_vars.colinear_value) {
            return false;
        }
