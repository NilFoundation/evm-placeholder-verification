...
library init_vars {
    ...
    struct vars_t_updated {
        types.placeholder_updated_proof_map proof_map;
        uint256 proof_size;
        types.transcript_data tr_state;
        types.fri_params_type fri_params;
        types.placeholder_common_data common_data;
    ...
    function init_updated(bytes calldata blob, uint256[] calldata init_params,
                       int256[][] calldata columns_rotations, vars_t_updated memory vars) internal view {

        (vars.proof_map, vars.proof_size) = placeholder_proof_map_parser.parse_updated_be(blob, 0);
        require(vars.proof_size == blob.length, "Proof length was detected incorrectly!");
        transcript.init_transcript(vars.tr_state, hex"");
