...
contract TestPlaceholderVerifierUpdatedMinaScalar {
    function verify(
        bytes calldata blob,
        // 0) modulus
        // 1) r
        ...
        init_vars.vars_t_updated memory vars;
        init_vars.init_updated(blob, init_params, columns_rotations, vars);
        ...
        local_vars.permutation_argument = permutation_argument.verify_eval_updated_be(blob, vars.tr_state,
        ...
        local_vars.gate_argument = mina_split_gen.evaluate_gates_be_updated(blob, gate_params, vars.common_data.columns_rotations);

        require(
            placeholder_verifier.verify_proof_updated_be(
                blob,
                vars.tr_state,
                vars.proof_map,
                vars.fri_params,
