...
const sar = async (hre) => {
    ...
    let tester;
    try {
        const SarTest = await ethers.getContractFactory("SarTest");
        tester = await SarTest.deploy();
    ...
    for (const [input, shift] of testCases) {
        try {
            tx = await tester.testSar(input, shift, { gasLimit: 1_000_000 });
