...
let t = x.then(result => {
    if (result === true) {
        fs.appendFileSync('time.log', 'placeholder-unified-addition: ' + Math.trunc(endTime - startTime).toString() + 'ms Verified ');
        myModule.estimateGas(contractAdress, contractAbi, proof).then(result => {fs.appendFileSync('time.log', result + '\n')});
        return "Verified!";
    } else {
        fs.appendFileSync('time.log', 'placeholder-unified-addition: ' + Math.trunc(endTime - startTime).toString() + 'ms Error ');
